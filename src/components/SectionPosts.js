        ...
        return (
            <section id={_.get(section, 'section_id', null)} className="posts">
                ...
                <Link key={post_idx} to={withPrefix(_.get(post, 'url', null))} className="article-teaser">
                    const thumbnail = render () {_.get(post, 'frontmatter.thumb_img_path', null) && (<img className="thumbnail" src={withPrefix(_.get(post, 'frontmatter.thumb_img_path', null))} alt={_.get(post, 'frontmatter.thumb_img_alt', null)} />)};
                    {thumbnail}
                    <div className="copy">
